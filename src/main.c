...
uint8_t xor_tables_data[NB_ROUNDS - 1][NB_COPY_XOR_TABLES]
                       [POSSIBLE_NIBBLES_VALUES][POSSIBLE_NIBBLES_VALUES]
    = { 0 };
...
print_tables (FILE *fd)
{
  ...
  fprintf (fd, "uint8_t xor_tables[%d][%d][%d][%d] = {\n", NB_ROUNDS - 1,
           NB_COPY_XOR_TABLES, POSSIBLE_NIBBLES_VALUES,
           POSSIBLE_NIBBLES_VALUES);
  for (int r = 0; r < NB_ROUNDS - 1; r++)
    {
      fprintf (fd, "\t{\n");
...
void
generate_random_mask (uint8_t mask[NB_ROUNDS])
{
          ...
          for (int x = 0; x < POSSIBLE_VALUES; x++)
            {
              tyi_tables_data[r][byte][0][x ^ mask[r]]
                  = (((gf_mul (2, x) << 24) | (x << 16) | (x << 8)
                      | gf_mul (3, x)))
                    ^ mask[r + 1];
              tyi_tables_data[r][byte][1][x ^ mask[r]]
                  = ((gf_mul (3, x) << 24) | (gf_mul (2, x) << 16) | (x << 8)
                     | x)
                    ^ mask[r + 1];
              tyi_tables_data[r][byte][2][x ^ mask[r]]
                  = ((x << 24) | (gf_mul (3, x) << 16) | (gf_mul (2, x) << 8)
                     | x)
                    ^ mask[r + 1];
              tyi_tables_data[r][byte][3][x ^ mask[r]]
                  = ((x << 24) | (x << 16) | (gf_mul (3, x) << 8)
                     | gf_mul (2, x))
                    ^ mask[r + 1];
            }
        }
